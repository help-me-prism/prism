...
try {
  ...
  // Research DB v2: always-on context panel, reading-time capture, curation queue promotion, and model-suggestion guard.
  await waitFor(() => notesConnection.evaluate(`document.querySelector('.notes-save-status')?.textContent.includes('저장됨')`), 'The paper note did not finish autosaving before capture.')
  await waitFor(() => notesConnection.evaluate(`Boolean(document.querySelector('.notes-context section[aria-label="백링크"]')) && Boolean(document.querySelector('.notes-context section[aria-label="인용 레이어"]'))`), 'The always-on context panel did not render backlinks and the citation layer for a paper note.')
  const captureResult = await notesConnection.evaluate(`window.prism.capturePaperNote({ kind: 'evidence', paperId: 'test.0001', anchorId: 'sentence-p1-1', memo: '노이즈 예측은 score matching이다 — 검증 필요' })`)
  assert(captureResult.blockId === 'evidence-test-0001-sentence-p1-1', `Reader capture did not return the evidence block id: ${JSON.stringify(captureResult)}`)
  ...
  await waitFor(async () => { const current = await fs.readFile(notePath, 'utf8'); return current.includes('노이즈 예측은 score matching이다 — 검증 필요') && current.includes('> [!ai]- AI 답변') && current.indexOf('## Notes') < current.indexOf('검증 필요') }, 'Reader and chat capture did not land inside the paper note Notes section.')
  await waitFor(() => notesConnection.evaluate(`document.querySelector('.cm-content')?.textContent.includes('검증 필요')`), 'The Notes editor did not reload the externally captured memo.')
  const contextPanelScreenshot = await notesConnection.send('Page.captureScreenshot', { format: 'png', captureBeyondViewport: false })
  await fs.writeFile(path.resolve('tmp/ui/notes-context-panel.png'), Buffer.from(contextPanelScreenshot.data, 'base64'))
  await notesConnection.evaluate(`document.querySelector('button[aria-label="정리 대기열"]').click()`)
  await waitFor(() => notesConnection.evaluate(`Boolean(document.querySelector('.curation-queue')) && document.querySelector('.curation-queue')?.textContent.includes('검증 필요')`), 'The curation queue did not list the captured memo for promotion.')
  const curationScreenshot = await notesConnection.send('Page.captureScreenshot', { format: 'png', captureBeyondViewport: false })
  await fs.writeFile(path.resolve('tmp/ui/notes-curation-queue.png'), Buffer.from(curationScreenshot.data, 'base64'))
  ...
  await waitFor(() => notesConnection.evaluate(`Boolean(document.querySelector('.curation-form input[aria-label="승격 노트 제목"]'))`), 'Choosing Claim promotion did not open the title form.')
  await notesConnection.evaluate(`(() => { const input = document.querySelector('.curation-form input[aria-label="승격 노트 제목"]'); const setter = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set; setter.call(input, '노이즈 예측은 가중 score matching이다 (스모크)'); input.dispatchEvent(new Event('input', { bubbles: true })); })()`)
  ...
  await waitFor(() => notesConnection.evaluate(`document.querySelector('.knowledge-heading h3')?.textContent.includes('(스모크)')`), 'Promoting a memo from the queue did not open the new Claim.')
  const promotedClaim = await fs.readFile(path.join(libraryPath, 'Claims', '노이즈 예측은 가중 score matching이다 (스모크).md'), 'utf8')
  assert(promotedClaim.includes('claim_origin: paper') && promotedClaim.includes('^evidence-test-0001-sentence-p1-1') && promotedClaim.includes('> [[papers/test.0001/test.0001|Editor fixture]]'), 'The promoted Claim lost its evidence card or source link.')
  await waitFor(async () => (await fs.readFile(notePath, 'utf8')).includes('검증 필요 → [[Claims/노이즈 예측은 가중 score matching이다 (스모크)|'), 'The paper note was not marked with the promoted Claim link.')
