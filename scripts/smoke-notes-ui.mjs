...
try {
  ...
  assert(JSON.stringify(paperRelationOptions) === JSON.stringify(['관련', '다룸', '제시함', '지지함', '반박함']), `The Paper relation picker exposed a noisy or incomplete relation model: ${JSON.stringify(paperRelationOptions)}`)
  await notesConnection.evaluate(`[...document.querySelectorAll('.relation-type-buttons button')].find((button) => button.textContent === '다룸').click()`)
  ...
  await notesConnection.evaluate(`[...document.querySelectorAll('.relation-type-buttons button')].find((button) => button.textContent === '관련').click()`)
