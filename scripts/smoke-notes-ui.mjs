...
try {
  ...
  await waitFor(() => notesConnection.evaluate(`Boolean(document.querySelector('.relation-picker'))`), 'The Paper relation picker did not open.')
  const paperRelationOptions = await notesConnection.evaluate(`[...document.querySelector('select[aria-label="지식 관계 유형"]').options].map((option) => option.value)`)
  assert(JSON.stringify(paperRelationOptions) === JSON.stringify(['related', 'discusses', 'presents', 'supports', 'contradicts']), `The Paper relation picker exposed a noisy or incomplete relation model: ${JSON.stringify(paperRelationOptions)}`)
  await notesConnection.evaluate(`(() => { const select = document.querySelector('select[aria-label="지식 관계 유형"]'); const setter = Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype, 'value').set; setter.call(select, 'discusses'); select.dispatchEvent(new Event('change', { bubbles: true })); })()`)
  ...
  await notesConnection.evaluate(`(() => { const select = document.querySelector('select[aria-label="링크 관계 유형"]'); const setter = Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype, 'value').set; setter.call(select, 'related'); select.dispatchEvent(new Event('change', { bubbles: true })); })()`)
  ...
  await waitFor(async () => (await fs.readFile(claimPath, 'utf8')).includes('[[Concepts/Reverse diffusion|Reverse diffusion]]') && (await notesConnection.evaluate(`document.querySelector('.knowledge-relations article small')?.textContent.includes('관련')`)), 'The knowledge picker did not save a portable link and selected relation together.')
  ...
  await notesConnection.evaluate(`(() => { const select = document.querySelector('select[aria-label="지식 관계 유형"]'); const setter = Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype, 'value').set; setter.call(select, 'related'); select.dispatchEvent(new Event('change', { bubbles: true })); })()`)
  await waitFor(() => notesConnection.evaluate(`document.querySelector('.relation-choice')?.textContent === '관련'`), 'The typed relation picker did not apply the selected relation type.')
  ...
  try {
    await waitFor(() => notesConnection.evaluate(`document.querySelector('.knowledge-relations article small')?.textContent.includes('관련')`), 'Creating a typed relation did not render its outgoing relation card.')
  } catch (reason) {
  ...
  const relationRecord = relationRecords.find((relation) => relation.sourceId === claimNodeId && relation.targetId === createdTypes[1] && relation.type === 'related')
  assert(relationRecord, 'The Claim-to-Concept related relation sidecar was not found.')
  assert(relationRecord.type === 'related' && relationRecord.creator === 'user' && relationRecord.reviewStatus === 'approved', 'The relation sidecar did not preserve type, creator, and review status.')
  assert((await fs.readFile(claimPath, 'utf8')).includes('> [!abstract] 관계 · 관련'), 'The approved relation was not kept as human-readable Markdown.')
  ...
  await waitFor(() => notesConnection.evaluate(`document.querySelector('.knowledge-heading h3')?.textContent === 'Reverse diffusion' && document.querySelector('.knowledge-relations article small')?.textContent.includes('관련')`), 'The target Concept did not show the relation as incoming.')
