...
try {
  ...
  await waitFor(() => notes.evaluate(`Boolean(document.querySelector('.notes-side'))`), 'The Notes window did not render its connections panel.')
  await notes.evaluate(`[...document.querySelectorAll('.tree-file')].find((button) => button.textContent.includes('Denoising'))?.click()`)
  await waitFor(() => notes.evaluate(`document.querySelector('.note-body .cm-content')?.textContent.includes('나중에 Claim으로')`), 'The paper note did not open with the captured memo.')
  await sleep(500)
  ...
  await notes.evaluate(`[...document.querySelectorAll('.tree-file')].find((button) => button.textContent.trim() === "Attention")?.click()`)
  await waitFor(() => notes.evaluate(`document.querySelector('.note-doc-title h1')?.textContent === 'Attention'`), 'The Attention concept did not open.')
  await sleep(500)
  await notes.shot('research-concept-definition-table.png')
  await notes.evaluate(`[...document.querySelectorAll('.side-chips button')].find((button) => button.textContent === '2홉')?.click()`)
  await sleep(700)
