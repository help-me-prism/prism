...
try {
  ...
  assert.equal(await evaluate('document.querySelectorAll(".paper-context-menu button[aria-pressed=true]").length'), 0)
  await evaluate('document.querySelector("button[aria-label=\\"새 대화\\"]").click()')
  assert.equal(await evaluate('document.querySelectorAll(".paper-context-menu button[aria-pressed=true]").length'), 0, 'An explicit empty paper scope survives a new chat')
  await evaluate('document.querySelector(".paper-tab i[title=\\"논문 닫기\\"]").click()')
  await wait('!document.querySelector(".paper-tab")')
  ...
  const inspected = JSON.parse(await fs.readFile(path.join(root, 'inspected-context.json'), 'utf8'))
  assert(inspected.prompt.includes('Cells respond to changes in their environment.'))
  assert(inspected.prompt.length <= 46000)
  assert.equal(inspected.libraryPath, vault)
  ...
  await evaluate('document.querySelector("button[aria-label=\\"설정 닫기\\"]").click()')
  await evaluate('document.querySelector(".reading-focus").click()')
  ...
  assert.equal(await evaluate('Boolean(document.querySelector(".figure-attachments, .figure-attachment"))'), false, 'Tagged figures must not create a persistent preview strip')
  assert(await evaluate('Boolean(document.querySelector(".composer-anchor .anchor-popover.image.figure img"))'), 'The figure tag must contain a hover-only thumbnail')
  await evaluate('document.querySelector(".composer-anchor .composer-anchor-label").click()')
  ...
  const persistedQuestion = await evaluate(questionText)
  await reload()
  await wait('Boolean(document.querySelector(".composer-editor") && document.querySelector(".continuous-page.rendered"))')
  await openCaptureSource(paper); await openCapture()
  assert.equal(await evaluate('document.querySelector("[aria-label=\\"노트 메모\\"]").value'), 'Biology draft round33', 'Renderer restart must restore source-owned memo drafts')
  assert.equal(await evaluate('document.querySelector("[aria-label=\\"정의하는 개념\\"]").value'), 'Biology concept round33')
  assert.equal(await evaluate(questionText), persistedQuestion, 'Renderer restart must restore the active chat draft')
  await openCaptureSource(external)
  ...
  await wait('window.prism.loadSessions().then(sessions => sessions.some(session => session.messages.some(message => message.id === "answer-navigation-answer" && message.savedNote)))')
  await reload()
  await wait('document.querySelector(".message-actions button")?.textContent.includes("저장한")')
  assert.equal((await fs.readFile(paper.notePath, 'utf8')).split(savedAnswerText).length - 1, 1, 'Restoring saved-answer state must not append the answer again')
  await evaluate('document.querySelector(".message-actions button").click()')
  let notesTarget
