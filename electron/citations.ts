 ...
 */
export type CitationEntry = { doi?: string; arxivId?: string; title: string; year?: number; citationCount?: number; authors: string[]; inLibrary: boolean; nodeId?: string }
...
type RawEntry = { doi?: string; arxivId?: string; title: string; year?: number; citationCount?: number; authors: string[] }
...
function parseEntries(payload: unknown, key: 'citedPaper' | 'citingPaper'): RawEntry[] {
  ...
  return data.flatMap((item) => {
    const paper = (item as Record<string, unknown> | undefined)?.[key] as { title?: string; year?: number; citationCount?: number; externalIds?: { ArXiv?: string; DOI?: string }; authors?: Array<{ name?: string }> } | undefined
    if (!paper?.title) return []
    return [{ doi: paper.externalIds?.DOI, arxivId: paper.externalIds?.ArXiv, title: paper.title, year: paper.year, citationCount: paper.citationCount, authors: (paper.authors ?? []).map((author) => author.name ?? '').filter(Boolean) }]
  })
}

async function fetchCitationGraph(arxivId: string, fetchImpl: FetchLike = fetch as unknown as FetchLike, externalId?: string): Promise<Omit<Stored, 'version'>> {
  ...
  const base = `https://api.semanticscholar.org/graph/v1/paper/${externalId ? encodeURIComponent(externalId) : `arXiv:${encodeURIComponent(arxivId)}`}`
 ...
 */
export async function listPaperCitations(libraryPath: string, arxivId: string, options: { refresh?: boolean; fetchImpl?: FetchLike; externalId?: string } = {}): Promise<CitationLinks> {
  if (arxivId.startsWith('local-') && !options.externalId) return { arxivId, fetchedAt: '', references: [], citations: [], stale: false, error: 'DOI 없이 가져온 PDF입니다. 인용 정보는 논문의 참고문헌에서 확인해 주세요.' }
  ...
  if (options.refresh === true || (options.refresh === undefined && isStale(stored))) {
    try {
      const fresh = await fetchCitationGraph(arxivId, options.fetchImpl, options.externalId)
  ...
  const doiNodes = new Map<string, string>()
  try {
    const records = JSON.parse(await fs.readFile(path.join(libraryPath, '.prism', 'library.json'), 'utf8')) as Array<{ arxivId: string; absUrl?: string }>
    for (const record of records) if (record.absUrl?.startsWith('https://doi.org/') && papers.has(record.arxivId)) doiNodes.set(record.absUrl.slice(16).toLowerCase(), papers.get(record.arxivId)!)
  } catch { /* arXiv-only vaults and older fixtures do not need this index */ }
  const enrich = (entry: RawEntry): CitationEntry => {
    const nodeId = (entry.arxivId ? papers.get(entry.arxivId) : undefined) ?? (entry.doi ? doiNodes.get(entry.doi.toLowerCase()) : undefined)
    return { ...entry, inLibrary: Boolean(nodeId), nodeId }
  }
