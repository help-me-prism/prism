...
const initialTemplates: Array<{ id: string; name: string; nodeType: KnowledgeNodeType; content: string }> = [
  // A default holds only what a person actually writes. Everything mechanical is added by the digest once it
  // has something to say, and the sections that are theirs are opened when they ask for one — so a new note
  // is never a page of empty headings. The long "정독 양식" forms that used to ship here were exactly that
  // page: the real vault had eight empty headings from them and nothing under any of them.
  ...
  { id: 'project-note', name: 'Project - 연구 노트', nodeType: 'project', content: '# {{title}}\n' },
]
...
async function ensureVault(libraryPath: string) {
  ...
  if (firstInitialization) {
    ...
    // Vaults made before the forms were cut still have the long ones on disk. They are left there — a
    // template the researcher edited is theirs — but the short ones are installed alongside and the defaults
    // point at those, so a new note starts as a title and nothing else.
    const migration = path.join(libraryPath, '.prism', 'migrations', 'light-defaults-v2')
    try { await fs.access(migration) } catch {
