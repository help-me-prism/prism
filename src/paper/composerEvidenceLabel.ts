...
export function composerEvidenceLabel(anchor: { label: string; type: string; page: number; source: string; paperTitle?: string; scientificSpans?: unknown }) {
  const kind = labels[anchor.type] ?? '근거'
  const location = Number.isInteger(anchor.page) && anchor.page > 0 ? `${anchor.page}쪽 · ${kind}` : kind
  const source = scientificPreviewText(anchor.source, anchor.scientificSpans).replace(/\s+/g, ' ').trim()
