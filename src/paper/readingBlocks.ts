...
export function groupReadingSegments<T extends { id: string; kind: string; blockId?: string }>(segments: T[], originalParagraphs = new Set<string>(), partition?: (segment: T) => string) {
  ...
  for (const segment of segments) {
    ...
    const key = `${original ? 'original' : segment.kind}-${segment.blockId ?? segment.id}${!original && partition ? '-' + partition(segment) : ''}`
