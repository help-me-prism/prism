...
type PaperFigureAsset = { id: string; order: number; caption?: string; sourcePath?: string; mimeType?: string; dataUrl?: string; compound?: boolean; components?: PaperFigureComponent[]; hasPanelLabels?: boolean }
type LatexBlock = { id: string; kind: 'heading' | 'paragraph' | 'caption' | 'equation' | 'figure' | 'table' | 'theorem'; source: string; section?: string }
type LatexStructure = { version: 6; rootFile: string; generatedAt: string; format?: 'latex' | 'jats'; provider?: 'arxiv' | 'europe-pmc'; license?: string; blocks: LatexBlock[] }
...
type WorkspaceSnapshot = { library: PaperRecord[]; openPaperIds: string[]; activePaperId?: string; activePage?: number; libraryPath?: string }
...
interface Window {
  prism: {
    ...
    openPaperUrl: (url: string) => Promise<void>
    ...
    importLocalPaper: (metadata?: ArxivPaper, file?: File) => Promise<PaperRecord | null>
    ...
    capturePaperView: (rect: { x: number; y: number; width: number; height: number }) => Promise<string>
    ...
    findLocalPaperLatex: (identity: { arxivId?: string; title: string }) => Promise<ArxivPaper | null>
    readPaperAnalysis: (arxivId: string, signature: string) => Promise<{ segments: TranslationSegment[]; matched: number } | null>
    savePaperAnalysis: (arxivId: string, signature: string, source: { segments: TranslationSegment[]; matched: number }) => Promise<boolean>
    readTranslationStatus: (arxivId: string) => Promise<import('../electron/translationStatus').TranslationStatus<TranslationSegment> | null>
