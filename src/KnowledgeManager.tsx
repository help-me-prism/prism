...
const relationLabels: Record<KnowledgeRelationType, string> = { discusses: '다룸', presents: '제시함', supports: '지지함', contradicts: '반박함', extends: '확장함', uses: '사용함', explains: '설명함', evidence_for: '근거임', derived_from: '출발함', raises: '질문 제기', related: '관련' }
const primaryRelationTypes: KnowledgeRelationType[] = ['related', 'discusses', 'presents', 'supports', 'contradicts', 'extends']
function relationTypesFor(source: KnowledgeNodeRecord, target: KnowledgeNodeRecord): KnowledgeRelationType[] {
  if (source.nodeType === 'paper' && target.nodeType === 'paper') return ['related']
  if (source.nodeType === 'paper' && target.nodeType === 'concept') return ['discusses']
  if (source.nodeType === 'paper' && target.nodeType === 'claim') return ['presents', 'supports', 'contradicts']
  if (source.nodeType === 'claim' && target.nodeType === 'claim') return ['supports', 'contradicts', 'extends']
  if (source.nodeType === 'concept' && target.nodeType === 'concept') return ['related']
  return ['related']
}
...
export default function KnowledgeManager({ onClose, initialNodeId }: { onClose: () => void; initialNodeId?: string }) {
  ...
  const availableRelationTypes = useMemo(() => active ? primaryRelationTypes.filter((type) => nodes.some((node) => node.id !== active.id && relationTypesFor(active, node).includes(type))) : ['related'] as KnowledgeRelationType[], [active, nodes])
  ...
  async function openNode(id: string, force = false) {
    ...
    try {
      ...
      setActiveId(id); setSnapshot(next); setContent(next.content); setBacklinks(nextBacklinks); setRelations(nextRelations); setDirty(false); setCreating(false); setDeleteReady(false); setLinkOpen(false); setRelationOpen(false); setGraphOpen(false); setSuggestionsOpen(false); setSectionTemplateOpen(false); setCopying(undefined); setCopyQuery(''); setClaimingEvidence(undefined); setEvidenceClaimQuery(''); setSectionTemplateId(suitable.find((template) => template.id === nextNode?.templateId)?.id ?? suitable.find((template) => template.isDefault)?.id ?? suitable[0]?.id ?? ''); setDataViewOpen(false); setError('')
    } catch (reason) { setError(String(reason)) }
  }
  ...
  async function connectEvidenceClaim(target: KnowledgeNodeRecord) {
  ...
  function openRelationPicker(type: KnowledgeRelationType = 'related') {
  ...
  function runSlashAction(action: MarkdownSlashAction) {
    ...
    openRelationPicker(action === 'supports' ? 'supports' : action === 'contradicts' ? 'contradicts' : 'related')
  }
  async function addRelation(target: KnowledgeNodeRecord, type = relationType) {
    if (!active || !snapshot) return
    try {
