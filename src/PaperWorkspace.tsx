...
type PdfTextItem = { str: string; width: number; height: number; transform: number[]; hasEOL?: boolean }
...
function segmentsFromItems(page: number, items: PdfTextItem[]): TranslationSegment[] {
  ...
  for (let itemIndex = 0; itemIndex < items.length; itemIndex += 1) {
    ...
    if (previous && combined) {
      ...
      combined += joinHyphen ? '' : (columnReset || paragraphGap || headingBoundary ? '\n\n' : ' ')
    }
  ...
  return parts.map((part, index) => ({
    id: `p${page}-s${index}-${shortHash(part.text)}`, page, source: part.text,
...
function translationLines(segment: TranslationSegment, itemRects: ItemRect[]) {
  const rects = (segment.itemIndexes ?? []).map((index) => itemRects[index]).filter(Boolean).sort((a, b) => a.top - b.top || a.left - b.left)
...
function PdfPage({ document: pdfDocument, pageNumber, scale, segments, translation, mode, highlighted, figureSelect, onHighlight, onTag, onVisible, onFigure }: {
  ...
  return <div className={`continuous-page ${mode}`} ref={pageRef} data-page={`${mode}-${pageNumber}`} style={pageSize}><canvas ref={canvasRef} />
    {mode === 'translated' && <div className="translated-text-layer">{translatedSegments.filter((segment) => segment.kind === 'text' && segment.translation).flatMap((segment) => translationLines(segment, itemRects).map((line, index) => <span key={`${segment.id}-${index}`} className={segment.id === highlighted ? 'highlighted' : ''} style={line.box} onMouseEnter={() => onHighlight(segment.id)} onMouseLeave={() => onHighlight(undefined)} onClick={() => onTag(segment)}>{line.text}</span>))}</div>}
    <div className="anchor-layer">{segments.flatMap((segment) => (segment.itemIndexes ?? []).map((itemIndex) => { const rect = itemRects[itemIndex]; return rect ? <span key={`${segment.id}-${itemIndex}`} className={segment.id === highlighted ? 'highlighted' : ''} style={rect} title="클릭하여 채팅에 태그" onMouseEnter={() => onHighlight(segment.id)} onMouseLeave={() => onHighlight(undefined)} onClick={() => onTag(segment)} /> : null }))}</div>
...
export default function PaperWorkspace({ providers, onToggleSidebar, onTagAnchor }: { providers: ProviderInfo[]; sidebarOpen: boolean; onToggleSidebar: () => void; onTagAnchor: (anchor: ContextAnchor) => void }) {
  ...
  const [translating, setTranslating] = useState(false); const [translationProgress, setTranslationProgress] = useState(''); const [figureSelect, setFigureSelect] = useState(false)
  ...
  useEffect(() => {
    ...
    const offProgress = window.prism.onTranslationProgress((payload) => { const event = payload as { arxivId?: string; completed?: number; total?: number; segments?: TranslationSegment[] }; if (event.arxivId === activeIdRef.current && event.segments) { setTranslation(event.segments); setTranslating(true); setTranslationProgress(`${event.completed}/${event.total}`) } })
    const offDone = window.prism.onTranslationDone((payload) => { const event = payload as { arxivId?: string; segments?: TranslationSegment[] }; if (event.arxivId === activeIdRef.current) { if (event.segments) setTranslation(event.segments); setTranslating(false); setTranslationProgress('완료') } })
  ...
  useEffect(() => {
    ...
    let disposed = false; setPageNumber(1); setAllSegments([]); setTranslation([]); setError(''); setViewMode('original')
    window.prism.readPaperPdf(activePaper.arxivId).then(async (data) => {
      ...
      if (disposed) return
      setAllSegments(segments); void window.prism.savePaperAnchors(activePaper.arxivId, segments)
      const cache = await window.prism.readTranslation(activePaper.arxivId); if (disposed) return
      if (cache?.segments.length) { setTranslation(cache.segments); setViewMode('dual') }
      else if (settings.autoTranslate && translationProvider?.available && !autoStartedRef.current.has(activePaper.arxivId)) {
        autoStartedRef.current.add(activePaper.arxivId); setTranslating(true); setTranslationProgress('준비 중…'); setViewMode('dual')
        void window.prism.startTranslation(activePaper.arxivId, segments).catch((reason) => { setTranslating(false); setError(reason instanceof Error ? reason.message : String(reason)) })
      }
  ...
  async function startTranslation() { if (!activePaper || !allSegments.length) return; setTranslating(true); setTranslationProgress('준비 중…'); setViewMode('dual'); try { await window.prism.startTranslation(activePaper.arxivId, allSegments) } catch (reason) { setTranslating(false); setError(reason instanceof Error ? reason.message : String(reason)) } }
  ...
  return <section className="reader-pane paper-workspace">
    ...
    {activePaper && pdf ? <><div className="paper-toolbar"><div className="page-nav"><button disabled={pageNumber <= 1} onClick={() => window.document.querySelector(`[data-page$="-${pageNumber - 1}"]`)?.scrollIntoView()}><ArrowLeft size={14} /></button><span>{pageNumber} / {pdf.numPages}</span><button disabled={pageNumber >= pdf.numPages} onClick={() => window.document.querySelector(`[data-page$="-${pageNumber + 1}"]`)?.scrollIntoView()}><ArrowRight size={14} /></button></div><div className="paper-title-mini"><strong>{activePaper.title}</strong><small>{activePaper.arxivId}{activePaper.sourcePath ? ' · LaTeX source 저장됨' : ''}</small></div><div className="reader-actions"><div className="document-mode"><button className={viewMode === 'original' ? 'active' : ''} onClick={() => setViewMode('original')}>원문</button><button className={viewMode === 'translated' ? 'active' : ''} onClick={() => setViewMode('translated')}>한국어</button><button className={viewMode === 'dual' ? 'active' : ''} onClick={() => setViewMode('dual')}><Columns2 size={13} /> 병기</button></div><select value={scale} onChange={(event) => setScale(Number(event.target.value))}><option value={.7}>70%</option><option value={.85}>85%</option><option value={1}>100%</option><option value={1.15}>115%</option></select><button className={figureSelect ? 'active' : ''} onClick={() => setFigureSelect((value) => !value)} title="PDF에서 영역을 드래그해 피겨로 저장하고 태그"><Image size={14} /> 피겨 태그</button><button onClick={() => onTagAnchor({ paperId: activePaper.arxivId, paperTitle: activePaper.title, anchorId: `p${pageNumber}`, type: 'page', page: pageNumber, label: `페이지${pageNumber}`, source: `Page ${pageNumber} of ${activePaper.title}` })}><Tag size={14} /> 페이지</button><button className={panel === 'notes' ? 'active' : ''} onClick={() => setPanel(panel === 'notes' ? null : 'notes')}><NotebookPen size={14} /> 노트</button></div></div>
      ...
      <div className={`paper-content document-layout ${panel ? 'with-sidecar' : ''} mode-${viewMode}`}>
        ...
        {(viewMode === 'translated' || viewMode === 'dual') && <div className="document-column translated-document"><header><Languages size={13} /> 한국어 문서 <span>{translating ? `번역 중 ${translationProgress}` : translation.length ? '저장됨' : '번역 대기'}</span></header><div className="document-scroll" ref={translatedScrollRef} onScroll={(event) => viewMode === 'dual' && syncScroll(event.currentTarget, sourceScrollRef.current)}>{pageRenderer('translated')}</div></div>}
